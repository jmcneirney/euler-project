...
use Pod::Usage;              # Note that doing this is just to do it not because it's the best way. A'ight?
...
my $problem = Problem->new({
  #  config   => 'config.yml',
    base_dir   => Path::Tiny->new('./euler-project'),
    uri        => URI->new("https://projecteuler.net/problem=22"),
    problem_id => '22',
});
